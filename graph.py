...
def main():
    ...
    splitter = CrossValidationSplitter(dataset, df)
    # criterion = torch.nn.CrossEntropyLoss()
    criterion = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(0.55))
    ...
    for _ in range(n_manytimes):
        fold_num = 0
        for train_dataset, test_dataset, train_df, test_df in splitter:
            fold_num += 1
            print("starting fold %d-%d" % (_, fold_num))
            ...
            for epoch in range(n_epoch):
                ...
                if epoch % 1 == 0:
                    ...
                    if early_stop_count > 3 and epoch>25:
                        if args.stage_two:
                            if auc_fold>0.5 and acc_fold > 0.5:
                                print('early stop at epoch %d' % epoch)
                                break
                        elif early_stop_count > 3:
                            print('early stop at epoch %d' % epoch)
                            break
            if auc_fold < 0.001 or c_index_fold < 0.01:
    ...
    total_count =  5 * n_manytimes - unsuccessful_count
