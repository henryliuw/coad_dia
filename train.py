...
def main():
    ...
    df_cached_file = os.path.join(args.data_dir, 'df.csv' % args.sample_n)
            ...
            for j in range(repl_n):
                ...
                if len(X_this)==args.sample_n:
                    ...
                    if Y is None:
                        ...
                        Y = np.r_[Y, Y_this]
                    if df_new is None:
                        df_new = pd.DataFrame({"y": Y_this, "time": useful_subset.loc[i, "OS.time"]}, index=[0])
                    else:
                        df_new = df_new.append({"y": Y_this, "time": useful_subset.loc[i, "OS.time"]}, ignore_index=True)
