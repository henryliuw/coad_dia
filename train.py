...
def main():
    ...
    if quick_load:
        ...
        with open(os.path.join(data_dir, 'X.pkl'),'rb') as file:
            X = pickle.load(file)
        with open(os.path.join(data_dir, 'Y.pkl'),'rb') as file:
            Y = pickle.load(file)
    else:
        useful_subset = pd.read_csv(csv_file)
        Y = useful_subset.loc[0, 'outcome'] == 'good'
        X = np.loadtxt(data_dir+'/'+'0_features.txt').reshape(1, 1000, 2048)
       
        size = len(useful_subset)
        for i in range(1, size):
            X_this = np.loadtxt(data_dir+'/'+str(i)+'_features.txt')
            if len(X_this)==1000:
                X = np.r_[X, X_this.reshape(1, 1000, 2048)]
    ...
    for i in range(5):
        ...
        model = Predictor(evidence_size=20)
        if gpu:
            model = model.cuda()
